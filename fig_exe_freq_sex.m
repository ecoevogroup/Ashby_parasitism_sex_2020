function fig_exe_freq_sex

% Generates Fig. 4 - epidemiological x epidemiological parameters
% (frequency of sex)
...
for panelcount=1:6
    ...
    if(exist(filename,'file'))
        ...
        for k1=1:asex_strain_total
            SEX_MEAN_FULL = cat(3,SEX_MEAN_FULL,repmat(SEX_MEAN(:,:,k1),[1,1,host_genotypes_grouped(k1,2)]));
        end
        subplot(2,3,panelcount)
        
        contourf(V2,V1,mean(SEX_MEAN_FULL,3),0:0.1:mx);
        set(gca,'clim',[0,mx])
...
temp(4) = temp(4) + 0.49;
set(C,'position',temp)
set(C,'ytick',0:0.2:0.8)
y2=ylabel(C,'frequency of sex','interpreter','latex','fontsize',14);
...
save2pdf('fig_exe_freq_sex.pdf',gcf,1200)